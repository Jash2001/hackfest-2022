...
function MydModalWithGrid(props) {
	const statement = {
		...
		2: ' Background: Department of Empowerment of Persons with Disabilities (DEPwD) under Ministry of Social Justice and Empowerment, Govt. Of India has already initiated a 24*7 Mental health Rehabilitation Helpline (KIRAN) for addressing the mental health issues within the population. It is now needed that an app (KIRAN app) is formulated to improve the accessibility of this services specially to adolescents across the country. Not only will this help in self registration but also in providing services to clients from the comfort of their home even if they are not ready to talk about it.Summary: screening the possible mental health issues in  adolescents and PwDs. Objective: <br>a) It will allow self registration screening of possible mental health issues <br>b) Initiate a secured chat in their own language with professional at any convenient time<br>c) Provide real time database, location, statistics and follow-up  services & utilization of local resources.<br>d) Provide an option for  call back at a convenient time slot chosen by the applicant',
